...
export function PropertyInspectionScheduler() {
  ...
  const { data: inspections = [], isLoading } = usePropertyInspections(parcel.id ?? null);
  const scheduleInspection = useScheduleInspection(parcel.id ?? null);
  ...
  const handleSchedule = () => {
    if (!newDate) return;
    scheduleInspection.mutate({ type: newType, scheduled_date: newDate, notes: "", priority: "normal" });
