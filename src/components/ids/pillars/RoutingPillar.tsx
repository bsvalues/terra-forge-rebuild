 ...
 export function RoutingPillar() {
   ...
   const recentEvents: RecentEvent[] = [
     {
       id: "1",
       type: "product.published (Parcel Fabric)",
       timestamp: new Date(Date.now() - 1000 * 60 * 30),
       impact: "Quantum Cockpit map refreshed",
       status: "success",
     },
     {
       id: "2",
       type: "product.published (Sales Stream)",
       timestamp: new Date(Date.now() - 1000 * 60 * 60 * 2),
       impact: "Ratio studies recalculated",
       status: "success",
     },
     {
       id: "3",
       type: "join.quality.changed",
       timestamp: new Date(Date.now() - 1000 * 60 * 60 * 4),
       impact: "Mismatch telemetry updated",
       status: "success",
     },
   ];
       ...
       <Card className="bg-tf-elevated/50 border-tf-border">
         <CardHeader>
           <div className="flex items-center justify-between">
             ...
             <Badge variant="outline" className="bg-tf-green/10 text-tf-green border-tf-green/30">
         ...
         <CardContent>
           <div className="space-y-3">
             {recentEvents.map((event) => (
               <div 
                 key={event.id}
                 className="flex items-center justify-between p-4 rounded-lg bg-tf-surface border border-tf-border"
               >
                 <div className="flex items-center gap-4">
                 ...
                 <span className="text-xs text-muted-foreground">
                   {Math.round((Date.now() - event.timestamp.getTime()) / 1000 / 60)} min ago
                 </span>
               </div>
             ))}
           </div>
