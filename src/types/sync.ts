...
export const SAGA_TEMPLATES: Record<SagaTemplate, Omit<SagaStep, "stepId" | "status">[]> = {
  ...
  pacs_migration: [
    { name: "Validate Config", action: "validate_config" },
    { name: "Extract Legacy", action: "extract_legacy" },
    { name: "Transform Schema", action: "transform_schema" },
    { name: "Load Records", action: "load_records", compensationAction: "rollback_load" },
    { name: "Verify Mapping", action: "verify_mapping" },
    { name: "Index & Optimize", action: "index_optimize" },
  ],
  sync_refresh: [
