...
-- FIX: Use jsonb_build_array() for safe array concatenation in health report
-- FIX: Align severity to use "error" consistently (FE will match)
...
begin
  ...
  -- Safe array appends using jsonb_build_array()
  ...
  if v_zero_coords > 0 then
    v_issues := v_issues || jsonb_build_array(jsonb_build_object('type','zero_coordinates','severity','warning','count',v_zero_coords,'description','Parcels with (0,0) placeholder coordinates'));
  end if;
