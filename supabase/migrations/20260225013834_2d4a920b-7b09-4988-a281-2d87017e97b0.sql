...
begin
  ...
  -- raw_present: any non-null lat/lng pair except (0,0)
  select count(*) into v_raw_present
  from parcels where county_id = v_county_id
    and latitude is not null and longitude is not null
    and not (latitude = 0 and longitude = 0);

  -- usable_wgs84: effective coordinate (canonical wins) is valid WGS84 within CONUS
  select count(*) into v_usable_wgs84
  from parcels where county_id = v_county_id
    and (
      -- canonical WGS84 within CONUS (canonical wins regardless of raw)
      ...
      or (
        -- no canonical, raw is valid degrees within CONUS
  ...
  -- null_coordinates: no coordinate data at all (raw or canonical)
  select count(*) into v_null_coords
  from parcels where county_id = v_county_id
    and latitude is null and longitude is null
    and latitude_wgs84 is null and longitude_wgs84 is null;

  -- zero_coordinates: explicit (0,0) placeholder in raw
  select count(*) into v_zero_coords
  from parcels where county_id = v_county_id
    and latitude = 0 and longitude = 0;

  -- convertible_wkid_2927: State Plane feet, no canonical yet
  ...
  -- invalid_wgs84: not valid degrees AND not a recognized projection, no canonical
  ...
  -- out_of_conus_bounds: effective coord (canonical wins) is valid WGS84 but outside CONUS
  -- If canonical exists and is in CONUS, this parcel is NOT flagged even if raw is out-of-CONUS
  select count(*) into v_out_of_bounds
  from parcels where county_id = v_county_id
    and (
      -- canonical exists AND is out of CONUS
      (latitude_wgs84 is not null and longitude_wgs84 is not null
       and not (latitude_wgs84 between 24.0 and 50.0 and longitude_wgs84 between -125.0 and -66.0))
      or (
        -- no canonical, raw is valid degrees but outside CONUS
        latitude_wgs84 is null and longitude_wgs84 is null
  ...
  -- duplicate coordinate groups: uses effective coord (canonical wins, else raw-as-degrees)
  ...
  -- backfill_eligible = raw_present (aligned definition)
  v_backfill_eligible := v_raw_present;

  -- Ratio-based severity for zero coordinates (denominator = total parcels = "% of roll poisoned")
  v_zero_severity := case
    when v_total > 0 and (v_zero_coords::numeric / v_total) > 0.1 then 'error'
    else 'warning'
  end;

  -- Build issues array (only emit when count > 0)
  ...
  return jsonb_build_object(
    ...
    'definitions', jsonb_build_object(
      'usable_wgs84', 'Effective coordinate (canonical WGS84 if present, else raw-as-degrees) within CONUS bounds (24-50°N, 66-125°W). Canonical wins: a parcel with valid canonical is usable even if raw is bad.',
      'raw_present', 'Any non-null raw lat/lng pair excluding (0,0) placeholders. Includes all SRIDs.',
      'convertible_wkid_2927', 'Raw coords in State Plane feet range (WKID 2927) without canonical WGS84 yet. Auto-convertible via backfill.',
      'null_coordinates', 'No raw or canonical coordinate data. Resolvable via polygon point-on-surface.',
      'out_of_conus_bounds', 'Effective coordinate is valid WGS84 degrees but falls outside CONUS. If canonical is in CONUS, parcel is NOT flagged even if raw is out-of-CONUS.',
      'duplicate_coordinate_groups', 'Groups sharing identical effective coordinates (canonical wins, else raw-as-degrees).',
      'zero_severity_denominator', 'total_parcels (county-wide roll risk, not just parcels-with-coords)',
      'effective_coord_rule', 'canonical (latitude_wgs84/longitude_wgs84) takes precedence over raw (latitude/longitude) for all classification and grouping'
    ),
    'sections', jsonb_build_object(
      'coordinate_quality', jsonb_build_object(
        'usable_wgs84', v_usable_wgs84,
        'raw_present', v_raw_present,
        'total_with_coords', v_raw_present + v_zero_coords,
