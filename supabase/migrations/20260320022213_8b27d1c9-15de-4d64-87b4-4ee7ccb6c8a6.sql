...
BEGIN
  -- NULL = global (all counties). Pass a specific UUID for county-scoped results.
  ...
  FROM parcels WHERE (v_county IS NULL OR county_id = v_county);

  -- Calibratable neighborhoods
  SELECT count(*) INTO v_calibratable_nbhd FROM (
    ...
    WHERE s.is_qualified = true AND s.sale_price > 0
      ...
      AND (v_county IS NULL OR p.county_id = v_county)
    GROUP BY p.neighborhood_code HAVING count(*) >= 10
  ) sub;

  -- Calibration stats
  SELECT count(DISTINCT neighborhood_code) INTO v_calibrated_nbhds
  FROM calibration_runs WHERE status = 'applied' AND (v_county IS NULL OR county_id = v_county);
  ...
  FROM sales WHERE (v_county IS NULL OR county_id = v_county);

  -- DQ issues
  SELECT count(*) FILTER (WHERE is_hard_blocker = true), count(*)
  INTO v_hard_blockers, v_open_issues
  FROM dq_issue_registry WHERE status = 'open' AND (v_county IS NULL OR county_id = v_county);

  -- Appeals
  SELECT count(*), count(*) FILTER (WHERE status = 'pending')
  INTO v_total_appeals, v_pending_appeals
  FROM appeals WHERE (v_county IS NULL OR county_id = v_county);
