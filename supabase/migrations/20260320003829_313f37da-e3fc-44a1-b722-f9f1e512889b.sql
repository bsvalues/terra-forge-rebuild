...
-- ============================================================
-- Spatial Nearest-Neighbor Neighborhood Backfill
-- For parcels that have coords but no neighborhood_code,
-- find the nearest parcel (by lat/lng distance) that DOES
-- have a neighborhood_code and assign it.
...
BEGIN
  WITH targets AS (
    SELECT id, latitude_wgs84, longitude_wgs84
    FROM parcels
  ...
  nearest AS (
    ...
    CROSS JOIN LATERAL (
      ...
      WHERE p2.county_id = p_county_id
        ...
        AND p2.latitude_wgs84 IS NOT NULL
        AND p2.longitude_wgs84 IS NOT NULL
        AND p2.id != t.id
      ORDER BY
        (p2.latitude_wgs84 - t.latitude_wgs84)^2 + 
        (p2.longitude_wgs84 - t.longitude_wgs84)^2
      LIMIT 1
    ) ref
  )
  UPDATE parcels p
