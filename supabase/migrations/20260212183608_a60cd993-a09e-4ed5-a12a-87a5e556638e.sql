CREATE OR REPLACE FUNCTION public.compute_ratio_statistics(p_tax_year integer DEFAULT (EXTRACT(year FROM CURRENT_DATE))::integer, p_sales_start_date date DEFAULT ((CURRENT_DATE - '2 years'::interval))::date, p_sales_end_date date DEFAULT CURRENT_DATE, p_neighborhood_code text DEFAULT NULL::text)
 RETURNS TABLE(sample_size integer, median_ratio numeric, mean_ratio numeric, cod numeric, prd numeric, prb numeric, low_tier_median numeric, mid_tier_median numeric, high_tier_median numeric, tier_slope numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  -- Try assessments table first, then fall back to parcels.assessed_value
  SELECT 
