...
-- Upgrade get_county_timeline with precise causal filters
CREATE OR REPLACE FUNCTION public.get_county_timeline(
  ...
  p_window_center timestamptz DEFAULT NULL,
...
BEGIN
  ...
  -- If window_center provided, override from/to with a tight window
  IF p_window_center IS NOT NULL THEN
    v_effective_from := p_window_center - (p_window_minutes || ' minutes')::interval;
    v_effective_to := p_window_center + (p_window_minutes || ' minutes')::interval;
  ELSE
  ...
  WITH unified AS (
    ...
    WHERE pe.county_id = v_county_id
      AND pe.started_at BETWEEN v_effective_from AND v_effective_to
    ...
    WHERE me.county_id = v_county_id
      AND me.created_at BETWEEN v_effective_from AND v_effective_to
    ...
    WHERE te.county_id = v_county_id
      AND te.created_at BETWEEN v_effective_from AND v_effective_to
    ...
    WHERE cr.county_id = v_county_id
      AND cr.created_at BETWEEN v_effective_from AND v_effective_to
    ...
    WHERE mr.created_at BETWEEN v_effective_from AND v_effective_to
      AND (mr.operator_id = auth.uid() OR mr.parcel_id IS NOT NULL)
  ),
  -- Apply link-based causal filter
  link_filtered AS (
    SELECT * FROM unified u
    WHERE (p_link_key IS NULL OR p_link_value IS NULL OR
           (u.links->>p_link_key) = p_link_value)
  ),
  filtered AS (
    SELECT * FROM link_filtered u
