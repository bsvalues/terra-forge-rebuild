...
BEGIN
  ...
  -- SECTION 1: Parcel Coordinates
  SELECT count(*), count(*) FILTER (WHERE latitude IS NOT NULL AND longitude IS NOT NULL),
    count(*) FILTER (WHERE latitude IS NULL OR longitude IS NULL),
    count(*) FILTER (WHERE latitude IS NOT NULL AND longitude IS NOT NULL AND (latitude < 24.0 OR latitude > 50.0 OR longitude < -125.0 OR longitude > -66.0)),
  ...
  FROM (SELECT count(*) AS dup_count FROM parcels WHERE county_id = v_county_id AND latitude IS NOT NULL AND longitude IS NOT NULL AND NOT (latitude = 0 AND longitude = 0) GROUP BY round(latitude::numeric, 6), round(longitude::numeric, 6) HAVING count(*) > 1) dups;

  SELECT count(*) FILTER (WHERE latitude IS NOT NULL AND (latitude < -90 OR latitude > 90)),
  ...
  IF v_out_of_bounds > 0 THEN
    v_coord_issues := v_coord_issues || jsonb_build_array(jsonb_build_object('type','out_of_bounds','severity','warning','count',v_out_of_bounds,'description',v_out_of_bounds||' parcels outside continental US bounds'));
  END IF;
  ...
  IF v_suspect_lat + v_suspect_lng > 0 THEN
    v_coord_issues := v_coord_issues || jsonb_build_array(jsonb_build_object('type','invalid_wgs84','severity','critical','count',v_suspect_lat+v_suspect_lng,'description','Coordinates outside valid WGS84 range — likely SRID mismatch'));
  END IF;
