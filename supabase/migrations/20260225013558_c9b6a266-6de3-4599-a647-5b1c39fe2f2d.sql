...
begin
  ...
  -- usable_wgs84: SQL-verified valid WGS84 degrees within CONUS bounds
  -- This is THE canonical "map-ready" count. No UI math needed.
  select count(*) into v_usable_wgs84
  from parcels where county_id = v_county_id
    and (
      -- canonical WGS84 fields populated AND within CONUS
      ...
      or (
        -- no canonical yet, but raw coords are valid WGS84 degrees within CONUS
  ...
  -- zero_coordinates: explicit (0,0) placeholder
  ...
  -- invalid_wgs84: not valid degrees AND not a recognized projection
  ...
  -- out_of_conus_bounds: valid WGS84 degrees but outside CONUS
  -- Checks both canonical AND raw-as-degrees (if no canonical yet)
  select count(*) into v_out_of_bounds
  from parcels where county_id = v_county_id
    and (
      -- canonical exists but outside CONUS
      (latitude_wgs84 is not null and longitude_wgs84 is not null
  ...
  -- duplicate coordinate groups (canonical only)
  ...
  if v_out_of_bounds > 0 then
    v_issues := v_issues || jsonb_build_array(jsonb_build_object(
      ...
      'description','Valid WGS84 degrees but outside continental US bounds'));
  end if;
  if v_dup_groups > 0 then
    v_issues := v_issues || jsonb_build_array(jsonb_build_object(
      'type','duplicate_coordinates','severity','info','count',v_dup_groups,
      'description','Groups of parcels sharing identical WGS84 coordinates'));
  end if;
  ...
  return jsonb_build_object(
    ...
    'definitions', jsonb_build_object(
      'usable_wgs84', 'Canonical WGS84 populated OR raw coords are valid degrees within CONUS bounds (24-50°N, 66-125°W). Excludes (0,0), State Plane, and out-of-CONUS.',
