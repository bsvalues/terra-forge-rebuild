...
BEGIN
  ...
  -- Calibratable neighborhoods (≥10 qualified sales)
  ...
  -- DQ
  SELECT count(*) FILTER (WHERE status = 'open' AND is_hard_blocker = true),
         count(*) FILTER (WHERE status = 'open')
  INTO v_hard_blockers, v_open_issues FROM dq_issue_registry;
