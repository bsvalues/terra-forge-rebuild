"""
Seed Clark County (Phase 186) — Generic ArcGIS + suggest_vendor() auto-detect
-------------------------------------------------------------------------------
Clark County uses Harris Govern PACS (inst. 2008) — same vendor as Benton.
Schema alignment with Benton is expected, but we use suggest_vendor() to
confirm and auto-detect in case field names diverge on the public ArcGIS layer.

Usage:
  python scripts/seed_clark.py --dry-run --limit 1000
  python scripts/seed_clark.py --out clark_sample.json
  python scripts/seed_clark.py --probe
...
def _summary_stats(rows: list[dict[str, Any]]) -> dict[str, Any]:
    vals = [
        r.get("market_value") or r.get("Mkt_Val") or r.get("TOTALVALUE")
        for r in rows
    ]
...
def run_seed(
    *,
    ...
    """
    Seed Clark County. Uses suggest_vendor() to confirm Harris PACS alignment
    (or fall back to generic_arcgis if the public layer diverges).
    ...
    try:
        ...
        print(f"[seed_clark] Probing: {url}")

        try:
            meta = probe_service(url)
        except Exception as probe_err:
            print(f"[seed_clark] WARNING - probe failed: {probe_err}")
            blob = _stub_output(str(probe_err))
            if out_path:
                Path(out_path).write_text(json.dumps(blob, indent=2), encoding="utf-8")
                print(f"[seed_clark] Wrote stub -> {out_path}")
            return 0

        fields = [f["name"] for f in meta.get("fields", [])]
        print(f"[seed_clark] Layer: {meta.get('name')} | fields: {len(fields)}")

        loader = FieldAliasLoader()

        if vendor is None:
            vendor = loader.suggest_vendor(fields)
            print(f"[seed_clark] Auto-detected vendor: {vendor}")
        else:
            print(f"[seed_clark] Using vendor hint: {vendor}")

        diff = loader.schema_diff(fields, vendor=vendor)
        coverage = diff.get("coverage_pct")
        print(f"[seed_clark] Schema coverage ({vendor}): {coverage}%")

        unmatched = diff.get("unmatched", [])
        if unmatched:
            print(f"[seed_clark] Unmatched fields ({len(unmatched)}): {unmatched[:10]}")

        if probe_only:
            print("[seed_clark] --probe flag set; skipping row fetch.")
            return 0

        try:
            rows = fetch_all(url, limit=limit)
        except Exception as fetch_err:
            print(f"[seed_clark] WARNING - fetch failed: {fetch_err}")
        ...
        if out_path:
            ...
            print(f"[seed_clark] Wrote sample -> {out_path}")

        print(
            f"[seed_clark] Fetched {len(rows)} rows; "
            f"normalised {len(norm)} rows (vendor: {vendor})"
        )
        return 0

    except Exception as e:
        print(f"[seed_clark] ERROR: {e}")
        return 1


def _cli() -> None:
    p = argparse.ArgumentParser(description="Seed Clark County (generic ArcGIS path)")
