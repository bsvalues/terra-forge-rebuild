"""
Seed Thurston County (Phase 186) — Generic ArcGIS + suggest_vendor() auto-detect
----------------------------------------------------------------------------------
Thurston County uses Aumentum Ascend (admin) + Sigma (appraisal). Vendor family
is shared with Yakima/Spokane, but field names may differ. This seeder uses
suggest_vendor() to auto-detect the best alias mapping rather than hardcoding.

Usage:
  python scripts/seed_thurston.py --dry-run --limit 1000
  python scripts/seed_thurston.py --out thurston_sample.json
  python scripts/seed_thurston.py --probe
...
def _summary_stats(rows: list[dict[str, Any]]) -> dict[str, Any]:
    vals = [
        r.get("market_value") or r.get("TOTALVALUE") or r.get("AppraisedValue")
        for r in rows
    ]
...
def run_seed(
    *,
    ...
    """
    Seed Thurston County. Uses suggest_vendor() to auto-detect the best alias
    mapping when vendor is not explicitly supplied.
    ...
    try:
        ...
        print(f"[seed_thurston] Probing: {url}")

        try:
            meta = probe_service(url)
        except Exception as probe_err:
            print(f"[seed_thurston] WARNING - probe failed: {probe_err}")
            blob = _stub_output(str(probe_err))
            if out_path:
                Path(out_path).write_text(json.dumps(blob, indent=2), encoding="utf-8")
                print(f"[seed_thurston] Wrote stub -> {out_path}")
            return 0

        fields = [f["name"] for f in meta.get("fields", [])]
        print(f"[seed_thurston] Layer: {meta.get('name')} | fields: {len(fields)}")
        ...
        if vendor is None:
            ...
            print(f"[seed_thurston] Auto-detected vendor: {vendor}")
        else:
            print(f"[seed_thurston] Using vendor hint: {vendor}")

        diff = loader.schema_diff(fields, vendor=vendor)
        coverage = diff.get("coverage_pct")
        print(f"[seed_thurston] Schema coverage ({vendor}): {coverage}%")

        unmatched = diff.get("unmatched", [])
        if unmatched:
            print(f"[seed_thurston] Unmatched fields ({len(unmatched)}): {unmatched[:10]}")

        if probe_only:
            print("[seed_thurston] --probe flag set; skipping row fetch.")
            return 0

        try:
            rows = fetch_all(url, limit=limit)
        except Exception as fetch_err:
            print(f"[seed_thurston] WARNING - fetch failed: {fetch_err}")
        ...
        if out_path:
            ...
            print(f"[seed_thurston] Wrote sample -> {out_path}")

        print(
            f"[seed_thurston] Fetched {len(rows)} rows; "
            f"normalised {len(norm)} rows (vendor: {vendor})"
        )
        return 0

    except Exception as e:
        print(f"[seed_thurston] ERROR: {e}")
        return 1


def _cli() -> None:
    p = argparse.ArgumentParser(description="Seed Thurston County (generic ArcGIS path)")
